...
{#if end != LineEnd.None}
	<path class="line-end end"  d={lineEndPaths.get(end)([x2, y2])} stroke-width={strokeWidth} stroke="currentColor" fill="none"/>
{/if}

{#if start != LineEnd.None}
	<path class="line-end start" d={lineEndPaths.get(start)([x1, y1])} stroke-width={strokeWidth} stroke="currentColor" fill="none"/>
{/if}