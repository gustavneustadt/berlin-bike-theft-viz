...
*/
	...
	export let data: DOMStringMap = undefined;
	
	const stepData: DOMStringMap[] = []
	const steps = [];
	const threshold = [];
	...
	const createObserver = (node: HTMLElement, index: number) => {
		const handleIntersect = (e: IntersectionObserverEntry[]) => {
			const ratio = e[0].intersectionRatio;
			steps[index] = ratio;
			stepData[index] = node.dataset
			mostInView()
		};
